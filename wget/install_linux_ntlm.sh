...
if [ ! -f "$sourceDir/VSCode-linux-x64.tar.gz" ]; then
	curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" https://az764295.vo.msecnd.net/stable/899d46d82c4c95423fb7e10e68eba52050e30ba3/code-stable-x64-1639562789.tar.gz -o "$sourceDir/VSCode-linux-x64.tar.gz"
fi
...
if [ ! -f "$sourceDir/ms-python-release.vsix" ]; then
	...
	curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://github.com/microsoft/vscode-python/releases/download/2021.11.1422169775/ms-python-release.vsix -o "$sourceDir/ms-python-release.vsix"
...
if [ ! -f "$sourceDir/robotframework-lsp.vsix" ]; then
	...
	curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://open-vsx.org/api/robocorp/robotframework-lsp/0.31.0/file/robocorp.robotframework-lsp-0.31.0.vsix -o "$sourceDir/robotframework-lsp.vsix"
...
if [ ! -f "$sourceDir/JavaScriptSnippets.vsix" ]; then
	curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://open-vsx.org/api/xabikos/JavaScriptSnippets/1.8.0/file/xabikos.JavaScriptSnippets-1.8.0.vsix -o "$sourceDir/JavaScriptSnippets.vsix"
...
if [ ! -f "$sourceDir/vscode-xml.vsix" ]; then
	curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://open-vsx.org/api/redhat/vscode-xml/0.18.1/file/redhat.vscode-xml-0.18.1.vsix -o "$sourceDir/vscode-xml.vsix"
...
if [ ! -f "$sourceDir/bash-ide-vscode.vsix" ]; then
	curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://open-vsx.org/api/mads-hartmann/bash-ide-vscode/1.11.0/file/mads-hartmann.bash-ide-vscode-1.11.0.vsix -o "$sourceDir/bash-ide-vscode.vsix"
...
if [ ! -f "$sourceDir/cpython-3.9.2-x86_64-unknown-linux-gnu-pgo-20210303T0937.tar.zst" ]; then
	...
	do
	   ...
	   curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://github.com/indygreg/python-build-standalone/releases/download/20210303/cpython-3.9.2-x86_64-unknown-linux-gnu-pgo-20210303T0937.tar.zst -o "$sourceDir/cpython-3.9.2-x86_64-unknown-linux-gnu-pgo-20210303T0937.tar.zst" && break
	   sleep 30
	   false
	done
fi
...
curl --proxy-ntlm -x $proxy -U $proxy_user:"$proxy_pass" -L https://bootstrap.pypa.io/get-pip.py -o "$sourceDir/get-pip.py"
