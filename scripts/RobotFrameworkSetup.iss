...
//
// If pip_requirements.txt exists in WheelDir (generated by build script),
// it will be used directly. Otherwise, falls back to scanning .whl files.
//
// Parameters:
...
begin
  ...
  SrcReqFile := WheelDir + '\pip_requirements.txt';
  ...
  // Check if pre-built pip_requirements.txt exists (generated by build script)
  // This file is already deduplicated and ready to use.
  UsePrebuilt := FileExists(SrcReqFile);
  ...
  // Fallback: scan wheel files if no pre-built requirements file
  if not UsePrebuilt then
  begin
    Log('  No pip_requirements.txt found, scanning wheel files...');
